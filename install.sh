...
command -v mysql   >/dev/null || FALTA+=(mysql-client)
(( ${#FALTA[@]} )) && { echo "${Y}⚠ instale antes de migrar: ${FALTA[*]}${N}"; }

echo "${G}✔ Instalado.${N} Comando disponível: ${B}migrar-grafana${N}"

# ---------------------------------------------------------------- assistente
# Sem terminal disponível (automação/CI) ou NO_WIZARD=1: encerra aqui.
[[ "${NO_WIZARD:-0}" == "1" || ! -e "$TTY" ]] && exit 0
{ exec 3< "$TTY"; } 2>/dev/null || exit 0   # abre o terminal UMA vez (fd 3)
